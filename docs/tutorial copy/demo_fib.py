...
def create_basic_fibonacci_flow(limit=10):
    ...
    def check_limit_post(shared, prep_res, exec_res):
        ...
        if shared["current_index"] < limit:
            return "continue"  # Keep going
        else:
            print(f"Fibonacci sequence generated with {len(shared['sequence'])} terms")
...
def create_fibonacci_batch_flow():
    ...
    # Create a batch flow with a Fibonacci node
    fib_node = create_fibonacci_node()
    batch_flow = BatchFlow(start=fib_node)
    
    # Prepare batch parameters for different sequence lengths
    ...
    # Display the results
    def post_process(shared, prep_res, exec_res):
    ...
    # Configure batch flow
    batch_flow.prep = prep_batch
    
    # Store results in shared context for post-processing
    def store_results(shared, prep_res, exec_res):
        shared["results"] = exec_res
        return "default"
    
    fib_node.post = store_results
    batch_flow.post = post_process
...
def create_parallel_fibonacci_flow():
    ...
    def exec_parallel(limits):
        print("Executing parallel Fibonacci sequence generation...")
    ...
    def post_parallel(shared, prep_res, exec_res):
        print("\nFibonacci Sequences Generated (Parallel):")
